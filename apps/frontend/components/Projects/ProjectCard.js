...
const ProjectCard = ({ title, subtitle, description, status, animationData }) => {
  ...
  return (
    <div className="relative w-full h-[30rem] md:h-[30rem] lg:h-[34rem] p-6 md:p-8 lg:p-12 mt-2 bg-gradient-to-tl from-[#DBD6FF] to-[#F4F3FF] rounded-2xl shadow-lg flex flex-col justify-between gap-6 transition-all duration-300 overflow-visible">
      {/* Background Lottie Animation */}
      <div
        ref={animationContainer}
        className="absolute bottom-0 right-0 w-full h-[14rem] md:w-full md:h-[20rem] lg:w-full lg:h-[24rem] object-contain"
      />
